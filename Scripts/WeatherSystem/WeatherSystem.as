...
// --- Storm popup synced vars ---
const string s_storm_msg       = "s_storm_msg";
const string s_storm_msg_start = "s_storm_msg_start";
const u16    STORM_DURATION    = 8 * getTicksASecond(); // 8 seconds
...
void onInit(CRules@ this)
{
    if (isServer())
    {
        ...
        // init storm popup vars (NEW)
        this.set_string(s_storm_msg, "");
        this.set_u32(s_storm_msg_start, 0);
        this.Sync(s_storm_msg, true);
        this.Sync(s_storm_msg_start, true);
    }
}
...
void ChangeRainLevel(CRules@ this, int newLevel)
{
    ...
    {
        this.set_string(s_storm_msg, "⚡ You hear an intense storm approaching ⚡");
        this.set_u32(s_storm_msg_start, getGameTime());
        this.Sync(s_storm_msg, true);
        this.Sync(s_storm_msg_start, true);
    }
...
// --- RENDER: draw the popup box for 8s if set (NEW) ---
void onRender(CRules@ this)
{
    const string msg = this.get_string(s_storm_msg);
    const u32 start  = this.get_u32(s_storm_msg_start);
    if (msg == "" || getGameTime() > start + STORM_DURATION) return;

    GUI::SetFont("menu");
    Vec2f center(getScreenWidth() / 2.0f, getScreenHeight() * 0.20f);

    Vec2f textSize;
    GUI::GetTextDimensions(msg, textSize);
    Vec2f tl = center - textSize * 0.5f - Vec2f(14, 10);
    Vec2f br = center + textSize * 0.5f + Vec2f(14, 10);

    GUI::DrawPane(tl, br, SColor(165, 10, 10, 10));                 // background
    GUI::DrawRectangle(tl - Vec2f(2,2), br + Vec2f(2,2),            // border
                       SColor(220, 255, 255, 0));
    GUI::DrawTextCentered(msg, center, SColor(255, 255, 240, 0));   // text
}
