...
void onTick(CBlob @ this)
{
	...
	if (ticks % u32(5 + 200 * mod_inv) == 0)
	{
		...
		if (getNet().isServer())
			this.server_Hit(this, this.getPosition(), Vec2f(0, 0), Maths::Min(ticks, 150) * 0.0015f * mod, Hitters::burn);
		if (getNet().isClient())
		{
			this.getSprite().PlaySound("/cough" + XORRandom(5) + ".ogg", 0.6f, this.getSexNum() == 0 ? 1.0f : 2.0f);
			if (this.isMyPlayer())
				ShakeScreen(60 * mod_inv, 5, this.getPosition());
		}
	}
