...
void onInit(CBlob @ this)
{
	...
	if (!this.exists("toxicity"))
		this.set_f32("toxicity", 0.50f);
...
{
	if (getNet().isServer() && this.getPosition().y < 0)
		this.server_Die();
	...
	if (this.getMap().getBlobsInRadius(this.getPosition(), this.getRadius() * 1.5f, @blobsInRadius))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			if (blob.hasTag("flesh") && !blob.hasTag("gas immune") && !blob.hasTag("zombie"))
			{
				...
				if (getNet().isServer())
					this.server_Hit(blob, blob.getPosition(), Vec2f(0, 0), 0.225f, Hitters::burn);
