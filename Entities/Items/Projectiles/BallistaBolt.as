...
bool doesCollideWithBlob(CBlob @ this, CBlob @blob)
{
	...
	if (carrier !is null)
		if (carrier.hasTag("player") && (this.getTeamNum() == carrier.getTeamNum() || blob.hasTag("temp blob")))
			return false;

	return (this.getTeamNum() != blob.getTeamNum() || blob.getShape().isStatic()) && blob.isCollidable();
}
...
void BallistaHitBlob(CBlob @ this, Vec2f hit_position, Vec2f velocity, const f32 damage, CBlob @blob, u8 customData)
{
	...
	if (DoExplosion(this, velocity) || this.get_bool("static"))
		return;
	...
	if (!blob.getShape().isStatic() || blob.getShape().getConsts().platform && !CollidesWithPlatform(this, blob, velocity))
		return;
...
void BallistaHitMap(CBlob @ this, const u32 offset, Vec2f hit_position, Vec2f velocity, const f32 damage, u8 customData)
{
	...
	else if (!map.isTileGroundStuff(type))
	{
		...
		if (blocks_pierced < 1 && speed > FAST_SPEED && map.isTileWood(type))
			this.set_u8("blocks_pierced", blocks_pierced + 1);
		else SetStatic(this, angle);
	}
