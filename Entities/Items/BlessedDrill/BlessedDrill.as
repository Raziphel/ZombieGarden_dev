...
const f32 speed_thresh = 30.0f;
const f32 speed_hard_thresh = 30.0f;

const string buzz_prop = " timer";

const string heat_prop = "blesseddrill heat";
const u8 heat_max = 120;

const u8 heat_add = 0;
const u8 heat_add_blob = heat_add * 0;
			...
			if (this.getMap().getHitInfosFromArc((this.getPosition() - attackVel), -attackVel.Angle(), 30, distance, this, false, @hitInfos))
			{
				...
				for (uint i = 0; i < hitInfos.length; i++)
				{
					...
					else															// map
					{
						...
						{
							TileType tile = map.getTile(hi.tileOffset).type;
