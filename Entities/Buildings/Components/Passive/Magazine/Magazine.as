...
{
	if (this.getDistanceTo(caller) > 16.0f || !this.getShape().isStatic())
		return;

	CBlob @carried = caller.getAttachments().getAttachmentPointByName("PICKUP").getOccupied();
	if (carried !is null)
	{
		if (carried.getRadius() > 3.5f || !carried.canBePutInInventory(this))
			return;

		CBlob @item = this.getInventory().getItem(0);
		if (item !is null && (item.getName() != carried.getName() || item.getQuantity() == item.maxQuantity))
			return;
...
void onCommand(CBlob @ this, u8 cmd, CBitStream @params)
{
	...
	if (cmd == this.getCommandID("load"))
	{
		...
		{
			// double check
			if (item.getName() != carried.getName() || item.getQuantity() == item.maxQuantity)
				return;
