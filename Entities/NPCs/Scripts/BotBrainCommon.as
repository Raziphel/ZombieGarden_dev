...
CBlob @getNewTarget(CBrain @ this, CBlob @blob, const bool seeThroughWalls = false, const bool seeBehindBack = false)
{
	...
	for (uint i = 0; i < players.length; i++)
	{
		...
		if (potential !is blob && blob.getTeamNum() != potential.getTeamNum() && (pos2 - pos).getLength() < 600.0f && (isBot || seeBehindBack || Maths::Abs(pos.x - pos2.x) < 40.0f || (blob.isFacingLeft() && pos.x > pos2.x) || (!blob.isFacingLeft() && pos.x < pos2.x)) && (isBot || seeThroughWalls || isVisible(blob, potential)) && !potential.hasTag("dead") && !potential.hasTag("migrant") && (!potential.hasTag("survivorplayer")))
		{
...
CBlob @getNewTargetStandGround(CBrain @ this, CBlob @blob, const bool seeThroughWalls = false, const bool seeBehindBack = false)
{
	...
	if (blob.getMap().getBlobsInRadius(pos, radius, @blobsInRadius))
	{
		...
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			Vec2f pos2 = potential.getPosition();
			// const bool isBot = blob.getPlayer() !is null && blob.getPlayer().isBot();
			if (potential !is blob && blob.getTeamNum() != potential.getTeamNum() && (!potential.hasTag("dead") && !potential.hasTag("migrant")) && (potential.hasTag("zombie") || potential.hasTag("undeadplayer")) && (!potential.hasTag("survivorplayer")) && (isBot || seeThroughWalls || isVisible(blob, potential)))
			{
...
bool JustGo(CBlob @blob, CBlob @target)
{
	...
	if (horiz_distance > blob.getRadius() * 0.75f)
	{
		...
		if (point.y + getMap().tilesize * 0.7f < mypos.y && (target.isOnGround() || target.getShape().isStatic()))
		{ // dont hop with me
...
bool isFriendAheadOfMe(CBlob @blob, CBlob @target, const f32 spread = 70.0f)
{
	...
	for (uint i = 0; i < players.length; i++)
	{
		CBlob @potential = players[i];
		Vec2f pos2 = potential.getPosition();
		if (potential !is blob && blob.getTeamNum() == potential.getTeamNum() && (pos2 - pos).getLength() < spread && (blob.isFacingLeft() && pos.x > pos2.x && pos2.x > targetPos.x) || (!blob.isFacingLeft() && pos.x < pos2.x && pos2.x < targetPos.x) && !potential.hasTag("dead") && !potential.hasTag("migrant"))
		{
