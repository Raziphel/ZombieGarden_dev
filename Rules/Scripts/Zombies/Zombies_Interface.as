...
int GetCurrentDay(CRules @rules)
{
	...
	return Maths::Max(
		1, d + (rules.exists("days_offset") ? rules.get_s32("days_offset") : 0));
}
...
// behaviour.
bool SafeGetSpawnSeconds(CRules @rules, const string&in propname,
						 u16&out seconds)
{
	seconds = 0;
	...
	if (candidate == 255 || candidate == 65535)
		return false;
...
void DrawRevivalTimer(CRules @rules, CPlayer @p)
{
	...
	// Center of screen, slightly above mid‑top so it doesn’t clash with your
	// stacked panels
	const Vec2f screen = getDriver().getScreenDimensions();
	const Vec2f center =
		Vec2f(screen.x * 0.5f, screen.y * (1.0f / 3.0f) + wobble);

	// Use your bold centered helper for consistent look; keep colors subtle so it
	// doesn’t scream
...
float DrawRecordStatus(CRules @rules)
{
	...
	const float margin = 24, padX = 10, padY = 8, lineH = 18, titleH = 20,
				titleGap = 6;
	...
	cols.push_back(COL_INFO);
	lines.insertLast("Zombies Kills: " + undeadKills);
	cols.push_back(COL_INFO);
...
float DrawZombiesHUDTopRight(CRules @rules, const float topOffset = 0.0f)
{
	...
	lines.insertLast("Zombies: " + (num_zombies + num_pzombies) + "/" +
					 max_zombies);
	...
	for (uint i = 0; i < lines.length(); i++)
	{
		...
		if (lines[i].findFirst("Zombies: ") == 0)
			...
			col = SColor(255, 160, 0, 255);	  // purple-ish
...
float DrawModeStatus(CRules @rules, const float topOffset = 0.0f)
{
	...
	{
		hardLine =
			"Hardmode in: " + hardRemain + (hardRemain == 1 ? " day" : " days");
		if (hardRemain <= 20)
			hardCol = COL_WARN;
		else
		{
	...
	{
		curseLine =
			"Curse in: " + curseRemain + (curseRemain == 1 ? " day" : " days");
		if (curseRemain <= 20)
			curseCol = COL_WARN;
		else
		{
	...
	const string ruinsLine =
		ruinsActive ? "Ruin Portals: ACTIVE" : "Ruin Portals: INACTIVE";
	...
	// --- lines ---
	Vec2f cur = Vec2f(tl.x + padX, tl.y + padY + titleH + titleGap);
	GUI::DrawText(hardLine, cur, hardCol);
	cur.y += lineH;
	GUI::DrawText(curseLine, cur, curseCol);
	cur.y += lineH;
	GUI::DrawText(ruinsLine, cur, ruinsCol);
	cur.y += lineH;
