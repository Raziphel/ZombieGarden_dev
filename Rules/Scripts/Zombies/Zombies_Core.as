		...
		if (allowWipeCheck)
		{
			...
			if ((live_survivors - num_hands) <= 0 && last_wipe_day != dayNumber)
			{
				...
				const float previewDifficulty = Maths::Min(
					20.0f, dayNumber * 0.5f + pillars * 0.2f - altars * 0.2f + survivors * 0.05f - undead * 0.2f + days_offset * 0.1f + wipeBonus);
				...
				if (zombiePlaces.length <= 0)
				{
					...
					for (int zp = 8; zp < 16; zp++)
					{
						...
						map.rayCastSolid(Vec2f(zp * 8, 0.0f),
										 Vec2f(zp * 8, map.tilemapheight * 8),
										 col);
						col.y -= 16.0f;
						zombiePlaces.push_back(col);
						map.rayCastSolid(
							Vec2f((map.tilemapwidth - zp) * 8, 0.0f),
							Vec2f((map.tilemapwidth - zp) * 8, map.tilemapheight * 8),
							col);
						col.y -= 16.0f;
						zombiePlaces.push_back(col);
					}
