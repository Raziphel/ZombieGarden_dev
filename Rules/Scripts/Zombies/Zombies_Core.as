...
class ZombiesCore : RulesCore {
  ...
  void Update() {
    ...
    float wipeBonus = rules.exists("difficulty_bonus")
                          ? rules.get_f32("difficulty_bonus")
                          : 0.0f;

    // --- Guard: don't let first-day / warmup wipes count ---
    const bool isLive = rules.isMatchRunning();
    const bool pastGrace =
        (timeElapsed > (getTicksASecond() * 45)); // small post-GAME grace
    ...
    if (allowWipeCheck) {
      ...
      const int last_wipe_day =
          rules.exists("last_wipe_day") ? rules.get_s32("last_wipe_day") : -1;
      ...
      if ((live_survivors - num_hands) <= 0 && last_wipe_day != dayNumber) {
        ...
        const float previewDifficulty = Maths::Min(
            20.0f, dayNumber * 0.5f + pillars * 0.2f - altars * 0.2f +
                       survivors * 0.05f - undead * 0.2f + days_offset * 0.1f +
                       wipeBonus);
      ...
      if (map !is null) {
        ...
        if (zombiePlaces.length <= 0) {
          ...
          for (int zp = 8; zp < 16; zp++) {
            Vec2f col;
            map.rayCastSolid(Vec2f(zp * 8, 0.0f),
                             Vec2f(zp * 8, map.tilemapheight * 8), col);
            col.y -= 16.0f;
            zombiePlaces.push_back(col);
            map.rayCastSolid(
                Vec2f((map.tilemapwidth - zp) * 8, 0.0f),
                Vec2f((map.tilemapwidth - zp) * 8, map.tilemapheight * 8), col);
            col.y -= 16.0f;
            zombiePlaces.push_back(col);
          }
        }
        ...
        const bool canSpawnNow =
            (hardmode || isNight) && (num_zombies < max_zombies);

        if (canSpawnNow) {
          float rmin =
              difficulty * 0.1f; // difficulty directly biases the minimum roll
          const float r =
              rmin +
          ...
          if (r >= 19.0f && _num_di < _max_di)
            ...
            server_CreateBlob(v == 0 ? "evilzombie"
                                     : (v == 1 ? "bloodzombie" : "plantzombie"),
                              -1, sp);
          } else if (r >= 1.0f) {
            const u8 v = XORRandom(2);
            ...
            const u8 v = XORRandom(2);
            server_CreateBlob((v == 0 ? "skeleton" : "skeleton2"), -1, sp);
          } else if (r >= 0.2f)
          ...
          // === boss waves ===
          int newTransition =
              RunBossWave(dayNumber, difficulty, zombiePlaces, transition);
  ...
  void onPlayerDie(CPlayer @victim, CPlayer @killer, u8 customData) {
    ...
    if (victim !is null && killer !is null &&
        killer.getTeamNum() != victim.getTeamNum()) {
