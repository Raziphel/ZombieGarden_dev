...
void onRenderScoreboard(CRules @ this) {
  ...
  makeWebsiteLink(Vec2f(150, getDriver().getScreenHeight() * 0.5f), "Discord",
                  "https://discord.gg/razi", controls, mousePos);
  mousePress = controls.mousePressed1;
}

void makeWebsiteLink(Vec2f pos, const string &in text, const string &in website,
                     CControls @controls, Vec2f &in mousePos) {
  GUI::SetFont("menu");
  Vec2f dim;
  ...
  const bool hover = (mousePos.x > tl.x && mousePos.x < br.x &&
  ...
  GUI::DrawTextCentered(
      text, Vec2f(tl.x + (width * 0.50f), tl.y + (height * 0.50f)), 0xffffffff);
}
