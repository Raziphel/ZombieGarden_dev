...
class ZombiesSpawns : RespawnSystem
{
	...
	bool canSpawnPlayer(PlayerInfo @p_info)
	{
		...
		if (info is null)
		{
			warn("Zombies LOGIC: Couldn't get player info ( in bool "
				 "canSpawnPlayer(PlayerInfo@ p_info) ) ");
			return false;
		}
	...
	Vec2f getSpawnLocation(PlayerInfo @p_info)
	{
		...
		if (p_info is null)
		{
			...
			if (m0 is null)
				return Vec2f(0, 0);
			f32 x0 =
				(XORRandom(2) == 0 ? 32.0f : m0.tilemapwidth * m0.tilesize - 32.0f);
			return Vec2f(x0,
						 m0.getLandYAtX(s32(x0 / m0.tilesize)) * m0.tilesize - 16.0f);
		}
		...
		f32 x =
			(XORRandom(2) == 0 ? 32.0f : map.tilemapwidth * map.tilesize - 32.0f);
		return Vec2f(x,
					 map.getLandYAtX(s32(x / map.tilesize)) * map.tilesize - 16.0f);
	}
	...
	void RemovePlayerFromSpawn(PlayerInfo @p_info)
	{
		...
		if (info is null)
		{
			warn("Zombies LOGIC: Couldn't get player info ( in void "
				 "RemovePlayerFromSpawn(PlayerInfo@ p_info) )");
			return;
		}
