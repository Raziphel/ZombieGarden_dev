...
#include "Hitters.as"
#include "GlobalPopup.as"   // <-- use the shared popup system
...
bool IsBlobUnderCover(CBlob@ blob, CMap@ map)
{
	...
	// robust width via bounding rect
	Vec2f tl, br;
	blob.getShape().getBoundingRect(tl, br);
	float halfWidth = (br.x - tl.x) * 0.5f * 0.9f;

	Vec2f skyTarget(myPos.x, SKY_Y_COORD);
	if (map.rayCastSolid(myPos, skyTarget)) return true;

	Vec2f leftFrom (myPos.x - halfWidth, myPos.y);
	Vec2f rightFrom(myPos.x + halfWidth, myPos.y);
	Vec2f leftTo  (myPos.x - halfWidth, SKY_Y_COORD);
	Vec2f rightTo (myPos.x + halfWidth, SKY_Y_COORD);

	if (map.rayCastSolid(leftFrom, leftTo))   return true;
	if (map.rayCastSolid(rightFrom, rightTo)) return true;
	return false;
}
...
void ChangeRainLevel(CRules@ this, int newLevel)
{
	...
	{
		this.set_u32(s_thunderstorm_cooldown, 30 * 60 * getTicksASecond()); // 30 minutes
	}
...
void onTick(CRules@ this)
{
	if (isServer())
	{
		...
		if (currentLevel > 0)
		{
			...
			if (currentLevel == 1) { max_angle_sway = 5.0f;  sway_speed = 0.002f; }
			else if (currentLevel == 2) { max_angle_sway = 15.0f; sway_speed = 0.004f; }
			else if (currentLevel == 3) { max_angle_sway = 30.0f; sway_speed = 0.008f; }
			else if (currentLevel == 4) { max_angle_sway = 45.0f; sway_speed = 0.015f; update_frequency = 8; }
		...
		const bool isForcingDown = this.get_bool(s_is_forcing_down);
		if (isForcingDown)
		{
			if (this.get_u32(s_state_timer) > 2 * 60 * getTicksASecond())
			{
				...
				{
					// keep storm for at least 30m if forced
				}
				else
				{
		...
		// --- State Machine ---
		if (currentLevel == 4) // thunderstorm
		{
		...
		else if (currentLevel == 3) // heavy rain
		{
			...
			if (this.get_u32(s_thunderstorm_cooldown) == 0 &&
			    this.get_u32(s_thunderstorm_check_timer) > 60 * getTicksASecond())
			{
				this.set_u32(s_thunderstorm_check_timer, 0);
		...
		else if (currentLevel > 0) // levels 1–2
		{
	...
	// --- Client: handle local rain SFX volume + script swapping ---
	if (isClient())
	{
		...
		if (blob.get_bool(FADE_FLAG_VAR))
		{
			...
			if (progress >= 1.0f)
			{
				...
				if (target_vol <= 0.0f) sprite.SetEmitSoundPaused(true);
			}
		}
	}
}
